...
public class UpdateEmployeeInfoForm {
	...
	private String CardId, employeeNumber;
	 ...
	 */
	public UpdateEmployeeInfoForm(employeeDTO employerDTO) {
		initialize(employerDTO);
	}
	 ...
	 */
	private void initialize(final employeeDTO employerDTO) {
		frame = new JFrame();
		...
		textField.setText(employerDTO.getEmployeeName());
		...
		employeeNumber = employerDTO.getEmployeeNumber();
		textField_1.setText(employerDTO.getEmployeeNumber());
		...
		CardId = employerDTO.getEmployeeCardId();
		textField_2.setText(employerDTO.getEmployeeCardId());
		...
		textField_4.setText(employerDTO.getEmployeeNativePlace());
		...
		textField_5.setText(employerDTO.getEmployeeDuty());
		...
		textField_6.setText(employerDTO.getEmployeePost());
		...
		textField_8.setText(employerDTO.getEmployeeTelephone());
		textField_8.setBounds(0, 288, 266, 36);
		panel_4.add(textField_8);

		comboBox = new JComboBox();
		comboBox.setFont(new Font("宋体", Font.PLAIN, 16));
		ComboxSimple c = new ComboxSimple(employerDTO.getDepartNumber(), comboBox);
		comboBox = c.getCombox();
		comboBox.setBounds(0, 249, 266, 42);
		panel_4.add(comboBox);

		datePickerBirthday = new JXDatePicker();
		datePickerBirthday.setBounds(0, 104, 266, 40);
		datePickerBirthday.setDate(employerDTO.getEmployeeBirthday());
		...
		final JCheckBox checkBox = new JCheckBox("\u7537");
		checkBox.setFont(new Font("宋体", Font.PLAIN, 16));
		checkBox.setBackground(Color.WHITE);
		checkBox.setBounds(120, 324, 54, 37);
		...
		checkBox_1.setBounds(185, 324, 54, 37);
		panel.add(checkBox_1);
		sex = new ButtonGroup();
		sex.add(checkBox);
		sex.add(checkBox_1);
		employeeId = employerDTO.getEmployeeId();
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (AddEmployeeLegal()) {
					...
					// 定义传输层对象 员工
					employeeDTO employerDTO_1 = new employeeDTO();
					// 定义部门编号
					DepartDTO departDTO = new DepartDTO();
					String departNumber = employerInfo
							.queryDepartNumberWhere(employerDTO.getDepartNumber(), (String) comboBox.getSelectedItem())
							.getDepartNumber();
					employerDTO_1.setDepartNumber(departNumber);
					employerDTO_1.setEmployeeName(textField.getText());
					employerDTO_1.setEmployeeNumber(textField_1.getText());
					employerDTO_1.setEmployeeCardId(textField_2.getText());
					employerDTO_1.setEmployeeSex(sex.getSelection() == checkBox.getModel() ? "男" : "女");
					SimpleDateFormat dateformat = new SimpleDateFormat("yyyy-MM-dd");
					employerDTO_1.setEmployeeBirthday(datePickerBirthday.getDate());
					employerDTO_1.setEmployeeNativePlace(textField_4.getText());
					employerDTO_1.setEmployeeDuty(textField_5.getText());
					employerDTO_1.setEmployeePost(textField_5.getText());

					employerDTO_1.setEmployeeTelephone(textField_8.getText());
					boolean savaFlag = true;

					if (employerInfo.queryEmployeeNumberIsExist(textField_1.getText()) != null) {
						if (employeeNumber.equals(textField_1.getText())) {
							savaFlag = savaFlag && true;
						} else {
							if (textField_1.getText().equals(employerInfo
									.queryEmployeeNumberIsExist(textField_1.getText()).getEmployeeNumber())) {
								JOptionPane.showMessageDialog(null, "该功号已经使用过了");
					...
					if (employerInfo.queryCardIdIsExist(textField_2.getText()) != null) {
						...
						if (CardId.equals(textField_2.getText())) {
							savaFlag = savaFlag && true;
						} else {
					...
					if (savaFlag) {
						...
						updateSuccessful = employerInfo.updateEmployeeinfo(employeeId, employerDTO_1);
						if (updateSuccessful) {
		...
		JLabel label_8 = new JLabel("\u4FEE\u6539\u5458\u5DE5\u4FE1\u606F");
